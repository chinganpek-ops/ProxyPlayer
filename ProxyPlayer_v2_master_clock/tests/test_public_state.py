...
def _stub_modules():
    ...
    for name in ("buffer", "buffer.frame_buffer", "decode", "decode.decoder",
                 ...
                 "file_io.win_sequential_reader", "config", "config.timebase",
                 "pipeline", "pipeline.adaptive_chunk", "pipeline.stream_scheduler",
                 "utils", "utils.utils", "utils.sync_logger"):
        mod(name)

    tb = mod("config.timebase")
    tb.FRAMES_PER_CHUNK = 12
    tb.SAMPLES_PER_CHUNK = 23040
    tb.SAMPLES_PER_VIDEO_FRAME = 1920
    tb.AUDIO_SAMPLE_RATE = 48000
    tb.video_frame_to_pts = lambda f: f * 1920
    tb.pts_to_video_frame = lambda p: p // 1920

    mod("buffer.frame_buffer").FrameRingBuffer = FakeBuffer
    mod("decode.decoder").Decoder = object
    mod("decode.audio_decoder").AudioDecoder = object
    mod("index.lazy_index").LazyIndex = object
    mod("index.lazy_index").IndexWindow = object
    mod("index.moov_builder").SEGMENT_SIZE = 4294967296
    mod("index.moov_builder")._abs_offset = lambda r: 0
    mod("file_io.win_sequential_reader").WinSequentialReader = object
    mod("utils.utils").get_real_size = lambda p: 0
    mod("utils.sync_logger").sync_monitor_logger = logging.getLogger("stub")
