...
Добавлен обратный вызов on_clock_update для обновления audio_clock в PlaybackEngine.
"""
...
class AudioOutput(QObject):
    ...
    def __init__(self, audio_buffers, parent=None, debug_callback=None, on_clock_update=None):
        super().__init__(parent)
    ...
    def _on_notify(self):
        try:
            if self._output and self._active:
                samples = self._device._samples_written
                self.audio_clock_changed.emit(samples)
                if self._on_clock_update:
                    self._on_clock_update(samples)
        except Exception as e:
