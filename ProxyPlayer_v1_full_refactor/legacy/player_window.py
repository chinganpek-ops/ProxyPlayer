#!/usr/bin/env python3
"""
player_window.py – главное окно плеера и менеджер окон.
Поддерживает многопроцессный режим с фиксированными позициями.
Менеджер самостоятельно разрешает ID файлов, переданных из Dalet,
используя homedir из конфигурации.
HTTP-обработка Video Helper полностью асинхронна (без блокировок).
Поиск файла только в корне homedir (без рекурсии), через glob.
Менеджер сворачивается в системный трей.
Ограничение на 6 окон плееров.
Панель управления по умолчанию скрыта.
Добавлены: поле ввода таймкода (по двойному клику), JKL-перемотка.
"""
...
from video_widget import GLVideoWidget
...
class PlayerWidget(QWidget):
    ...
    def __init__(self, mp4_path: Path, config: dict, use_moov: bool = False, parent=None):
        ...
        self.player.ready.connect(self._on_player_ready)
        self.player.error.connect(self._on_player_error)
        self.player.seek_finished.connect(self._on_seek_finished)
        if hasattr(self.player, 'audio_clock_changed'):
            self.player.audio_clock_changed.connect(self._on_audio_clock_changed)
        self.player.placeholder_hide.connect(self.video_widget.hide_placeholder)
        self.player.recording_finished.connect(self._on_recording_finished)
    ...
    def _create_player_controller(self):
        ...
        return PlayerController(
            ref_path=self.ref_path, idx_path=self.idx_path, mp4_path=self.mp4_path,
            ...
            normal_playback_speed_factor=self.config.get('normal_playback_speed_factor', 1.25),
            rate_limit=self.config.get('default_rate_limit', 0),
    ...
    def _enter_timecode_edit_mode(self, event: QMouseEvent = None):
        """Двойной клик по таймкоду – переход в режим редактирования."""
        # Ставим на паузу, если играет
    ...
    def _exit_timecode_edit_mode(self):
        """Выход из режима редактирования таймкода."""
        self.tc_input.hide()
        self.tc_label.show()
        self.setFocus()
    ...
    def start_playback(self):
        if not self.player._ready.is_set():
            self.player.ready.connect(self._delayed_start); return
        self._start_playback_now()

    def _delayed_start(self): self._start_playback_now()
    ...
    def _update_frame(self):
        ...
        if isinstance(self._active_player, PlayerController):
        ...
        # Обновление надписи на кнопке Play/Pause с учётом JKL
    ...
    def _on_audio_clock_changed(self, samples):
        if not self._seeking and isinstance(self._active_player, PlayerController):
            frame_idx = samples // 1920
    ...
    def _on_timecode_entered(self):
        """Перемотка к введённому таймкоду."""
        if not self.player:
            return
        ...
        if not tc_text or tc_text == "00:00:00;00":
            ...
            return
        try:
            from timebase import timecode_to_frame
    ...
    def _go_live(self):
        if not isinstance(self._active_player, PlayerController):
            return
        self._active_player.seek_to_live_position(offset_frames=1600)
        self.setFocus()
    ...
    def keyPressEvent(self, event):
        # Если поле ввода в фокусе – клавиши плеера не обрабатываем
...
class ManagerWindow(QMainWindow):
    def __init__(self, mp4_path: Path, config, use_moov: bool = False):
        ...
        self.setWindowTitle("Dalet Proxy Player v4 – Panel")
        ...
        self.tray_icon.setToolTip("Dalet Proxy Player")
        ...
        self.server.listen("DaletPlayerManager")

        # HTTP сервер для Video Helper
        ...
        # Сразу сворачиваем панель в трей
        self.hide()
    ...
    def _resolve_id_to_mp4(self, file_id: str) -> Path | None:
        ...
        logger.warning(f"Файл для ID {file_id} не найден в {home}")
        return None
    ...
    def _open_new_player(self):
        ...
        if start_dir and not os.path.isdir(start_dir):
            logger.warning(f"homedir '{start_dir}' не существует, используется директория по умолчанию")
            start_dir = ''
    ...
    def _cleanup_hwnd_positions(self):
        ...
        for hwnd in dead:
            ...
            logger.debug(f"Освобождена позиция HWND {hwnd}")
