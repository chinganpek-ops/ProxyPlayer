"""
seek_engine.py – асинхронный seek с отменой для ProxyPlayer v1.
Использует LazyIndex для быстрого поиска IDR и декодер для извлечения кадров.
"""
...
class SeekEngine:
    ...
    def seek_async(
        ...
        Вызывает on_complete(buffer) при успехе, on_error(msg) при ошибке.
        Возвращает объект SeekRequest для отслеживания/отмены.
        """
    ...
    def seek_sync(self, frame_idx: int) -> FrameRingBuffer:
        """Синхронный seek (блокирует поток)."""
        request = SeekRequest()
    ...
    def _seek_sync(self, frame_idx: int, request: SeekRequest) -> FrameRingBuffer:
        ...
        last_rec = window.video_records[end_local]

        from index.moov_builder import _abs_offset  # абсолютное смещение
        start_offset = int(_abs_offset(first_rec))
        end_offset = int(_abs_offset(last_rec))
        # Грубая оценка размера последнего кадра – до следующей записи или до mdat_end
